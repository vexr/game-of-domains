...
const main = async () => {
  ...
  const nameMatches = (s: string) =>
    /consensus|transfer|xdm|domain|evm|balance|balances/i.test(s);
  ...
  for (const c of uniqueCandidates.slice(0, 50)) {
    ...
    const typeFields: string[] =
      res.__type?.fields?.map((f: any) => f.name) ?? [];
    detailed.push({ name: c.name, typeName: c.typeName, fields: typeFields });
  }

  const likelyTransfers = detailed.filter((d) =>
    d.fields.some((f) =>
      ["from", "to", "from_chain", "to_chain", "timestamp"].includes(f)
    )
  );

  console.log(
    JSON.stringify({ endpoint, candidates: detailed, likelyTransfers }, null, 2)
  );
};
